function renderItems(items, processType, elementId, processFunction) {
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        placeholder += "<div class='itemContainer'><p>" + title + "</p><div class='actionButton' " + 'id="' + placeholderId + '">'
        + processType + '</div>' + "</div>";
    ...
    for (let i=0; i < itemsMeta.length; i++) {
        document.getElementById(itemsMeta[i]["id"]).addEventListener(
...
function editItem() {
    ...
    let call = apiCall("/api/v1/item/edit", "POST");
    let json = {
        "title": title,
        "status": "DONE"
    };
...
function deleteItem() {
    ...
    let call = apiCall("/api/v1/item/delete", "POST");
    let json = {
        "title": title,
        "status": "DONE"
    };
...
function getItems() {
    let call = apiCall("/api/v1/item/get", "GET");
    call.send();
}
...
function createItem() {
    ...
    let call = apiCall("/api/v1/item/create/" + title.value, "POST");
    call.send()
